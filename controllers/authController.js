...
async function login(req, res) {
  try {
    ...
    // Save refreshToken in DB for future token rotation
    user.refreshToken = refreshToken;
    await user.save();

    // ✅ Set Access Token in cookie (not httpOnly) so frontend JS can read it
    res.cookie('accessToken', accessToken, {
      maxAge: 15 * 60 * 1000, // 15 minutes
      httpOnly: false,        // So frontend can access it via document.cookie
    });

    // ✅ Set Refresh Token in cookie with only expiration (default flags)
    res.cookie('refreshToken', refreshToken, {
      maxAge: 4 * 24 * 60 * 60 * 1000, // 4 days
    });
